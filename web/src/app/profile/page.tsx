...
export default function ProfilePage() {
  ...
  return (
    <AppShell>
      <div style={{ maxWidth: 520, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 14, padding: '24px 16px 110px' }}>
        <div
          className="animate-fade-up"
          style={{ position: 'relative', borderRadius: 26, overflow: 'hidden', background: '#fff', border: '1px solid #EDEDF1', boxShadow: '0 1px 4px rgba(20,20,25,0.03)' }}
        >
          {photoUrls.length > 0 ? (
            <img
              key={photoIndex}
        ...
        <div className="animate-fade-up" style={{ borderRadius: 20, padding: '20px 20px', background: '#fff', border: '1px solid #EDEDF1', boxShadow: '0 1px 4px rgba(20,20,25,0.03)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <h2 style={{ fontSize: 14, fontWeight: 800, color: '#151515', margin: 0, letterSpacing: 1.4, textTransform: 'uppercase' }}>About</h2>
            <button onClick={() => router.push('/edit-profile')} aria-label="Edit about" style={{ width: 32, height: 32, borderRadius: 9999, background: '#F3F3F6', border: '1px solid #EDEDF1', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <PencilIcon size={14} color="#8A8A8F" />
            </button>
          </div>
          <p style={{ fontSize: 15, color: profile?.bio ? '#151515' : '#8A8A8F', lineHeight: '24px', margin: '12px 0 0' }}>
        ...
        <div className="animate-fade-up" style={{ borderRadius: 20, padding: '20px 20px', background: '#fff', border: '1px solid #EDEDF1', boxShadow: '0 1px 4px rgba(20,20,25,0.03)' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
            <h2 style={{ fontSize: 14, fontWeight: 800, color: '#151515', margin: 0, letterSpacing: 1.4, textTransform: 'uppercase' }}>Interests</h2>
            <button onClick={() => router.push('/edit-profile')} aria-label="Edit interests" style={{ width: 32, height: 32, borderRadius: 9999, background: '#F3F3F6', border: '1px solid #EDEDF1', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <PencilIcon size={14} color="#8A8A8F" />
            </button>
          </div>
          {(profile?.interests?.length || 0) > 0 ? (
            <div style={{ marginTop: 14, display: 'flex', flexDirection: 'column', gap: 18 }}>
              {INTEREST_CATEGORIES
                ...
                .map(({ cat, items }) => (
                  <div key={cat.label}>
                    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 1.2, textTransform: 'uppercase', color: '#8A8A8F', marginBottom: 10 }}>
                      {cat.label}
                    </div>
            ...
            <button onClick={() => router.push('/edit-profile')} style={{ background: 'none', border: 'none', padding: 0, cursor: 'pointer' }}>
              <p style={{ fontSize: 14.5, color: '#8A8A8F', margin: '12px 0 0', textAlign: 'left' }}>
        ...
        <div className="animate-fade-up" style={{ borderRadius: 20, overflow: 'hidden', background: '#fff', border: '1px solid #EDEDF1', boxShadow: '0 1px 4px rgba(20,20,25,0.03)' }}>
          <div style={{ padding: '16px 20px 0' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <h2 style={{ fontSize: 14, fontWeight: 800, color: '#151515', margin: 0, letterSpacing: 1.4, textTransform: 'uppercase' }}>Other Personal Details</h2>
              <button onClick={() => router.push('/edit-profile')} aria-label="Edit personal details" style={{ width: 32, height: 32, borderRadius: 9999, background: '#F3F3F6', border: '1px solid #EDEDF1', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <PencilIcon size={14} color="#8A8A8F" />
              </button>
            </div>
          </div>
          {[
            ...
            <button
              ...
              style={{
                ...
                background: 'transparent', border: 'none', borderBottom: i < arr.length - 1 ? '1px solid #EDEDF1' : 'none',
                cursor: 'pointer', textAlign: 'left', gap: 12,
              }}
            >
              <span style={{ width: 130, fontSize: 11, fontWeight: 700, letterSpacing: 1.2, textTransform: 'uppercase', color: '#8A8A8F', flexShrink: 0 }}>{row.label}</span>
              <span style={{ flex: 1, fontSize: 15, fontWeight: 600, color: row.value === '-' ? '#8A8A8F' : '#151515', textTransform: row.capitalize ? 'capitalize' : 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row.value}</span>
        ...
        <div className="animate-fade-up" style={{ borderRadius: 20, overflow: 'hidden', background: '#fff', border: '1px solid #EDEDF1', boxShadow: '0 1px 4px rgba(20,20,25,0.03)' }}>
          {detailsRows.map((row, i) => (
            <button
              ...
              style={{
                width: '100%', display: 'flex', alignItems: 'center', padding: '15px 20px',
                background: 'transparent', border: 'none', borderBottom: i < detailsRows.length - 1 ? '1px solid #EDEDF1' : 'none',
                cursor: 'pointer', textAlign: 'left', gap: 12,
              }}
            >
              <span style={{ width: 108, fontSize: 11, fontWeight: 700, letterSpacing: 1.2, textTransform: 'uppercase', color: '#8A8A8F', flexShrink: 0 }}>{row.label}</span>
              <span style={{ flex: 1, fontSize: 15, fontWeight: 600, color: '#151515', textTransform: row.capitalize ? 'capitalize' : 'none', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{row.value}</span>
              <ChevronForwardIcon size={16} color="#65656A" />
            </button>
          ))}
        </div>
        ...
        <div
          ...
          style={{
            ...
            background: superlikes.remaining > 0
              ...
              : '#fff',
            border: superlikes.remaining > 0 ? '1px solid rgba(79,195,247,0.35)' : '1px solid #EDEDF1',
            boxShadow: superlikes.remaining > 0 ? undefined : '0 1px 4px rgba(20,20,25,0.03)',
          }}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 15, fontWeight: 800, color: '#151515' }}>Super Likes</div>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ fontSize: 15, fontWeight: 800, color: '#151515' }}>Likes</div>
              <div style={{ fontSize: 12.5, color: '#8A8A8F', marginTop: 2 }}>
                {likes.isPremium
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 18 }}>
                {[
                  ...
                  <div key={i} style={{ flex: 1, borderRadius: 14, padding: '12px 6px', background: '#F8F8FA', border: '1px solid #EDEDF1', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 7 }}>
                    ...
                    <span style={{ fontSize: 10.5, fontWeight: 700, color: '#65656A', textAlign: 'center', lineHeight: '13px', whiteSpace: 'pre-line' }}>{b.label}</span>
                  </div>
                ))}
              </div>
          ...
          {menuItems.map((item, i) => (
            <button
              ...
              style={{
                ...
                borderRadius: 16, gap: 13, border: '1px solid #EDEDF1', cursor: 'pointer', textAlign: 'left', width: '100%',
                background: '#fff',
                boxShadow: '0 1px 4px rgba(20,20,25,0.03)',
                animation: `fadeUp 0.45s ease both`, animationDelay: `${i * 0.05}s`,
              }}
            >
              <div style={{ width: 38, height: 38, borderRadius: 12, background: '#F3F3F6', border: '1px solid #EDEDF1', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {item.icon}
              </div>
              <span style={{ flex: 1, fontSize: 14.5, fontWeight: 600, color: '#151515' }}>{item.label}</span>
              <ChevronForwardIcon size={16} color="#65656A" />
            </button>
          ))}
        </div>
