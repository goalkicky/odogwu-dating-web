    ...
    >
      <div
        ...
        style={{
          ...
          border: '1px solid rgba(61,252,119,0.35)',
          boxShadow: '0 24px 80px rgba(0,0,0,0.7), 0 0 60px rgba(61,252,119,0.12)',
        }}
      >
        <div style={{ position: 'absolute', top: -70, left: -50, width: 220, height: 220, borderRadius: 9999, background: 'rgba(61,252,119,0.18)', filter: 'blur(60px)', pointerEvents: 'none' }} />
        ...
        <div style={{ position: 'relative', zIndex: 1, padding: '38px 24px 30px' }}>
          <div style={{ width: 84, height: 84, borderRadius: 26, background: 'linear-gradient(135deg, #3DFC77, #22E5FF)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px', boxShadow: '0 12px 40px rgba(61,252,119,0.5)', animation: 'floaty 3.5s ease-in-out infinite' }}>
          ...
          <p style={{ fontSize: 14.5, color: '#ABABAB', lineHeight: '22px', margin: '10px auto 0', maxWidth: 320 }}>
            Don&apos;t wait for a match to break the ice. Premium members can <b style={{ color: 'white' }}>message anyone first</b> Ã¢â‚¬â€ make the first move and stand out instantly. Ã°Å¸â€™Â¬
          </p>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 22 }}>
            {PLANS.map((p) => (
              <div key={p.name} style={{ flex: 1, position: 'relative', borderRadius: 14, padding: '12px 6px', background: 'rgba(255,255,255,0.05)', border: `1px solid ${p.popular ? 'rgba(255,230,0,0.5)' : 'rgba(255,255,255,0.1)'}`, boxShadow: p.popular ? '0 6px 20px rgba(255,230,0,0.15)' : 'none' }}>
                ...
                <div style={{ fontSize: 11, color: '#FF7BA0', fontWeight: 700, marginTop: 3 }}>Message First</div>
              </div>
            ))}
          </div>
          ...
          <button
            ...
            style={{
              ...
              background: 'linear-gradient(135deg, #3DFC77, #22E5FF)',
              color: 'white', fontSize: 16, fontWeight: 800, letterSpacing: 0.3,
              boxShadow: '0 12px 34px rgba(61,252,119,0.4)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9,
            }}
          >
            <DiamondIcon size={19} color="white" />
            Get Premium Ã¢â‚¬â€ from N4,900/mo
          </button>
