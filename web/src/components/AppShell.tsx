...
export default function AppShell({ children, header }: { children: React.ReactNode; header?: React.ReactNode }) {
  ...
  const desktopNav = [
    ...
    { href: '/matches', label: 'Messages', icon: <span style={{ position: 'relative', display: 'inline-flex', lineHeight: 0 }}><svg viewBox="0 0 48 48" className="uv-nav-svg"><path d="M24 40s-14-9-14-20a8 8 0 0 1 14-5 8 8 0 0 1 14 5c0 11-14 20-14 20Z" fill="none" strokeWidth="2.4"/></svg>{messagesCount > 0 && <b style={{ position: 'absolute', top: -8, right: -12, background: '#D90040', color: '#fff', borderRadius: '50%', minWidth: 17, height: 17, fontSize: 10, fontWeight: 700, display: 'grid', placeItems: 'center', padding: '0 3px', boxSizing: 'border-box' }}>{messagesCount >= 100 ? '99+' : messagesCount}</b>}</span> },
  ...
  return (
    <div className="uv-shell">
      <style jsx global>{`
        .uv-shell { background: #ececec; min-height: 100svh; color: #111111; font-family: -apple-system, BlinkMacSystemFont, "SF Pro Display", "SF Pro Text", "Inter", Arial, Helvetica, sans-serif; }
        .uv-shell a { text-decoration: none; }

        .uv-brand { display: flex; align-items: center; gap: 9px; }
        .uv-brand-mark { width: 66px; height: 66px; border: 7px solid #E50046; border-radius: 50%; position: relative; flex-shrink: 0; box-sizing: border-box; object-fit: cover; display: block; }
        .uv-brand-mark:before { content: ""; position: absolute; width: 18px; height: 18px; border: 6px solid #fff; border-radius: 50%; background: #E50046; left: -5px; top: -5px; box-sizing: border-box; }
        ...
        .uv-brand-name { font-size: 42px; line-height: 39px; font-weight: 800; letter-spacing: -2px; color: #E50046; }
        .uv-brand-sub { text-align: center; font-size: 13px; font-weight: 700; letter-spacing: 6px; margin-top: 8px; }
        .uv-brand-sub b { color: #E50046; letter-spacing: 0; }
        .uv-brand-sub span { color: #E50046; }
        ...
        .uv-menu span { display: block; width: 34px; height: 3px; background: #111111; margin: 8px 0; border-radius: 2px; }
        .uv-messages-top { position: relative; width: 50px; height: 50px; background: none; border: 0; cursor: pointer; color: #111111; padding: 0; }
        .uv-messages-top svg { width: 40px; height: 40px; }
        .uv-messages-top em { position: absolute; right: 0; top: -4px; background: #D90040; color: #fff; width: 24px; height: 24px; border-radius: 50%; font-style: normal; font-size: 13px; display: grid; place-items: center; font-weight: 700; }
        ...
        .uv-desktop-link {
          ...
          border-radius: 14px; color: #5D616A; font-size: 15px; font-weight: 600;
        }
        .uv-desktop-link:hover { background: #FFF0F4; color: #E50046; }
        .uv-desktop-link.active { background: #FFF0F4; color: #E50046; font-weight: 700; }
        ...
        .uv-sidebar-bottom a { color: #E50046; font-weight: 700; font-size: 13px; display: block; }
        ...
        .uv-bottom-nav {
          ...
          height: 60px; background: #fff; border-top: 1px solid #EBEBEE;
        ...
        <nav className="uv-bottom-nav">
          ...
          <Link href="/matches" className={`uv-bottom-link ${isActive('/matches') ? 'active' : ''}`}>
            <span style={{ position: 'relative', display: 'inline-flex', lineHeight: 0 }}>
              ...
              {messagesCount > 0 && <b style={{ position: 'absolute', top: -6, right: -10, background: '#D90040', color: '#fff', borderRadius: '50%', minWidth: 16, height: 16, fontSize: 9, fontWeight: 700, display: 'grid', placeItems: 'center', padding: '0 3px', boxSizing: 'border-box' }}>{messagesCount >= 100 ? '99+' : messagesCount}</b>}
