    ...
    >
      <div
        ...
        style={{
          ...
          boxShadow: '0 24px 80px rgba(0,0,0,0.7), 0 0 60px rgba(255,55,95,0.15)',
        }}
      >
        <div style={{ position: 'absolute', top: -70, left: -50, width: 220, height: 220, borderRadius: 9999, background: 'rgba(255,55,95,0.22)', filter: 'blur(60px)', pointerEvents: 'none' }} />
        ...
        <div style={{ position: 'relative', zIndex: 1, padding: '38px 24px 30px' }}>
          <div style={{ width: 84, height: 84, borderRadius: 26, background: 'linear-gradient(135deg, #FF375F, #FF3B30)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px', boxShadow: '0 12px 40px rgba(255,55,95,0.55)', animation: 'floaty 3.5s ease-in-out infinite' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 22 }}>
            {PLANS.map((p) => (
              <div key={p.name} style={{ flex: 1, position: 'relative', borderRadius: 14, padding: '12px 6px', background: 'rgba(255,255,255,0.05)', border: `1px solid ${p.popular ? 'rgba(255,215,0,0.5)' : 'rgba(255,255,255,0.1)'}`, boxShadow: p.popular ? '0 6px 20px rgba(255,215,0,0.15)' : 'none' }}>
                ...
                <div style={{ fontSize: 11, color: '#FF6B8A', fontWeight: 700, marginTop: 3 }}>∞ Likes</div>
              </div>
            ))}
          </div>
