...
const PLANS = [
  { name: 'Premium', count: '2/day', color: '#FF2E5F' },
  { name: 'Surplus', count: '5/day', color: '#FFE600', popular: true },
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div style={{ position: 'absolute', bottom: -80, right: -40, width: 220, height: 220, borderRadius: 9999, background: 'rgba(255,46,95,0.18)', filter: 'blur(60px)', pointerEvents: 'none' }} />
        ...
        <div style={{ position: 'relative', zIndex: 1, padding: '38px 24px 30px' }}>
          <div style={{ width: 84, height: 84, borderRadius: 26, background: 'linear-gradient(135deg, #22E5FF, #0AA6CE)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px', boxShadow: '0 12px 40px rgba(79,195,247,0.5)', animation: 'floaty 3.5s ease-in-out infinite' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 22 }}>
            {PLANS.map((p) => (
              <div key={p.name} style={{ flex: 1, position: 'relative', borderRadius: 14, padding: '12px 6px', background: 'rgba(255,255,255,0.05)', border: `1px solid ${p.popular ? 'rgba(255,230,0,0.5)' : 'rgba(255,255,255,0.1)'}`, boxShadow: p.popular ? '0 6px 20px rgba(255,230,0,0.15)' : 'none' }}>
                ...
                <div style={{ fontSize: 11, color: '#FFE600', fontWeight: 700, marginTop: 3 }}>{p.count}</div>
              </div>
            ))}
          </div>
