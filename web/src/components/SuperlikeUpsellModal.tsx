...
import { StarIcon, DiamondIcon, CloseIcon, CheckmarkCircleIcon } from '@/components/Icons';
...
export default function SuperlikeUpsellModal({ onClose }: { onClose: () => void }) {
    ...
    >
      <div
        ...
        style={{
          ...
          background: 'linear-gradient(165deg, #101C2E 0%, #14121A 55%, #1A1218 100%)',
          border: '1px solid rgba(79,195,247,0.35)',
          boxShadow: '0 24px 80px rgba(0,0,0,0.7), 0 0 60px rgba(79,195,247,0.15)',
        }}
      >
        <div style={{ position: 'absolute', top: -70, left: -50, width: 220, height: 220, borderRadius: 9999, background: 'rgba(79,195,247,0.18)', filter: 'blur(60px)', pointerEvents: 'none' }} />
        <div style={{ position: 'absolute', bottom: -80, right: -40, width: 220, height: 220, borderRadius: 9999, background: 'rgba(255,55,95,0.18)', filter: 'blur(60px)', pointerEvents: 'none' }} />
        ...
        <div style={{ position: 'relative', zIndex: 1, padding: '38px 24px 30px' }}>
          <div style={{ width: 84, height: 84, borderRadius: 26, background: 'linear-gradient(135deg, #4FC3F7, #0288D1)', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 18px', boxShadow: '0 12px 40px rgba(79,195,247,0.5)', animation: 'floaty 3.5s ease-in-out infinite' }}>
            ...
            {PLANS.map((p) => (
              <div key={p.name} style={{ flex: 1, position: 'relative', borderRadius: 14, padding: '12px 6px', background: 'rgba(255,255,255,0.05)', border: `1px solid ${p.popular ? 'rgba(255,215,0,0.5)' : 'rgba(255,255,255,0.1)'}`, boxShadow: p.popular ? '0 6px 20px rgba(255,215,0,0.15)' : 'none' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5 }}>
                  <StarIcon size={12} color={p.color} />
                  <span style={{ fontSize: 13, fontWeight: 800, color: 'white' }}>{p.name}</span>
                </div>
                <div style={{ fontSize: 11, color: '#FFD700', fontWeight: 700, marginTop: 3 }}>{p.count}</div>
              </div>
            ))}
          </div>
          ...
          <button
            ...
            style={{
              ...
              boxShadow: '0 12px 34px rgba(255,55,95,0.45)', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9,
            }}
          >
